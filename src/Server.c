...
static void MPConnection_FailConnect(cc_result result) {
	static const cc_string reason = String_FromConst("failed to connect to the server. prob down");
	cc_string msg; char msgBuffer[STRING_SIZE * 2];
	String_InitArray(msg, msgBuffer);
...
static void MPConnection_BeginConnect(void) {
	static const cc_string invalid_reason = String_FromConst("vro, Invalid IP address");
...
static void MPConnection_Disconnect(void) {
	...
	static const cc_string reason = String_FromConst("gng, You've lost connection to the server");
...
static void DisconnectReadFailed(cc_result res) {
	cc_string msg; char msgBuffer[STRING_SIZE * 2];
	String_InitArray(msg, msgBuffer);
	String_Format3(&msg, "well Error reading from %s:%i: %e" _NL, &Server.Address, &Server.Port, &res);
