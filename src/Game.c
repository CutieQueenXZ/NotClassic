...
extern void CuboidCommand_Tick(void);
...
static const short normalDists[] = {
...
/* Classic UI mode */
static const short classicDists[] = {
    8, 32, 128, 512
};
...
void Game_CycleViewDistance(void) {
    ...
    if (Gui.ClassicMenu) {
        arr = classicDists;
        count = Array_Elems(classicDists);
    } else {
        arr = normalDists;
        count = Array_Elems(normalDists);
    }

    if (Input_IsShiftPressed()) {
        CycleViewDistanceBackwards(arr, count);
    } else {
        CycleViewDistanceForwards(arr, count);
    }
}


cc_bool Game_ReduceVRAM(void) {
	if (Game_UserViewDistance <= 1) return false;
	Game_UserViewDistance /= 2;
	Game_UserViewDistance = max(1, Game_UserViewDistance);
...
static cc_bool Window_IsInactive(void) {
...
static void LoadOptions(void) {
	...
	Game_ViewDistance     = Options_GetInt(OPT_VIEW_DISTANCE, 8, 1000000, DEFAULT_VIEWDIST);
...
/*
 * Some ClassiCube ports (e.g. webclient) track inactive/background state via Window_Main.Inactive.
 * This source tree doesn't expose that field on all platforms (e.g. Linux/SDL).
 * Keep the event hook, but make the handler a no-op so the build stays portable.
 */
static void HandleInactiveChanged(void* obj) { (void)obj; }
...
void Game_RenderFrame(void) {
    ...
    #ifndef ANDROID
        #if defined(CC_BUILD_WEB) || defined(CC_BUILD_MOBILE)
            return;
        #endif
    #endif
#endif
