...
/* ===== Name mode names ===== */
const char* const NameMode_Names[NAME_MODE_COUNT] = {
    "None",
    "Hovered",
    "All",
    "AllHovered",
...
/* ===== Shadow mode names ===== */
const char* const ShadowMode_Names[SHADOW_MODE_COUNT] = {
    "None",
    "SnapToBlock",
    "Circle",
    "CircleAll"
...
static void EnvSettingsScreen_InitWidgets(struct MenuOptionsScreen* s) {
	MenuOptionsScreen_BeginButtons(s);
	{
		...
		MenuOptionsScreen_AddInt(s, "Clouds height", 
			-99999999, 99999999, World.Height + 2,
...
static int  MiO_GetCuboidDelay(void) {
	/* Stored as microseconds, but shown as milliseconds in the menu */
	int us = Options_GetInt(OPT_CUBOID_DELAY_US, 1000, 2000000, 40000);
	return us / 1000;
}
static void MiO_SetCuboidDelay(int ms) {
	if (ms < 1) ms = 1;
	Options_SetInt(OPT_CUBOID_DELAY_US, ms * 1000);
}
