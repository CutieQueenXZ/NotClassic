...
static void Menu_RenderBounds(void) {
	PackedCol topCol    = PackedCol_Make(0, 0, 0, 100);
	PackedCol bottomCol = PackedCol_Make(0, 0, 0, 100);
...
static void GenLevelScreen_Make(struct GenLevelScreen* s, int i, int def) {
	...
	if (i == 3) {
		...
		MenuInput_Int(desc, 1, 1661992959, def);
	}
...
static void TexPackOverlay_UpdateLine3(struct TexPackOverlay* s) {
	...
	if (s->deny) {
		TextWidget_SetConst(&s->lbls[3], "YOU don't want to download the texture pack???", &s->textFont);
		...
		TextWidget_SetConst(&s->lbls[3], "Download size: Unknown.. bet", &s->textFont);
	} else {
